...
function GamesPage({ params }: { params: Promise<{ eventId: string }> }) {
  const eventId = use(params).eventId;
  ...
  return (
    <div>
      <div>
        <button className="btn w-full" onClick={startGames}>
          Start Games
        </button>
      </div>
...
function ScoreTile({
  ...
  team: GameTeam;
  onPointScored: Function;
}) {
  return (
    <div
      className="h-1/2 w-full flex justify-center items-center text-[20vh] relative"
      onClick={() => onPointScored(team.name)}
    >
      {team.points}
      <div className="text-base font-bold absolute bottom-4 flex flex-col">
        {team.rotation.map((p, idx) => (
          <span key={p.id}>
            {idx + 1} {p.name}
          </span>
        ))}
