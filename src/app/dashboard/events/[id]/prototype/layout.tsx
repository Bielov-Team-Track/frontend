...
import { ArrowLeft, Calendar, Clock, CreditCard, Edit, MapPin, MessageCircle, MoreHorizontal, Settings, Share2, Trash2, Users, XCircle } from "lucide-react";
...
type TabType = "overview" | "teams" | "members" | "payments" | "settings";

const TABS: { id: TabType; label: string; icon: typeof Calendar; href: string }[] = [
...
export default function EventPrototypeLayout({ children }: { children: React.ReactNode }) {
	...
	const getTabHref = (tabId: TabType): string => {
		const base = `/dashboard/events/${eventId}/prototype`;
	...
	const isAdmin = true; // TODO: derive from user/event relationship
	const isOpen = !event?.canceled && new Date(event?.startTime || 0) > new Date();
	const isFull = totalSpots > 0 && totalParticipants >= totalSpots;
	const hasInvitation = false; // TODO: check if current user has pending invitation
