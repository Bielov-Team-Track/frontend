...
function ImageCropper({ imageFile, onImageSave }: { imageFile: File, onImageSave: (image: Blob) => Promise<void> }) {
  return (
    <div className='border p-4 rounded'>
      <p>Image selected: {imageFile.name}</p>
      <button className='btn btn-primary' onClick={() => onImageSave(imageFile)}>Save</button>
    </div>
  )
}
...
function ProfileCreate() {
  ...
  const updateProfileImage = async (_image: Blob, _userId: string) => { /* TODO: implement */ }
  const signup = async () => { /* TODO: implement */ }
  ...
  const uploadImage = async (image: Blob) => {
    ...
    updateProfileImage(image, userId).then(() => {
      ...
      (error?.response as any)?.data && setError((error.response as any).data.message)
      setIsLoading(false)
    })
  }
