    ...
    if (!coachProfile) {
         return (
             <div className="bg-surface border border-border rounded-2xl p-12 text-center flex flex-col items-center gap-4">
                ...
                <div>
                    <span className="text-lg font-medium text-foreground block">No Coach Profile</span>
                ...
                {coachProfile.qualifications && coachProfile.qualifications.length > 0 ? (
                    <div className="flex flex-col gap-3">
                        {coachProfile.qualifications.map((qual) => (
                            <div key={qual.id} className="flex items-center justify-between p-3 bg-surface rounded-xl border border-border">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-accent/10 flex items-center justify-center text-accent shrink-0">
                                        <Award size={16} />
                                    </div>
                                    <span className="text-foreground font-medium">{qual.name}</span>
                                </div>
                                {qual.year > 0 && (
