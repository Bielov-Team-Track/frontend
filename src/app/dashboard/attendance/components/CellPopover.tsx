...
import { Calendar, AlertTriangle, Clock, DollarSign, Gift } from "lucide-react";
...
export default function CellPopover({
    ...
    const [status, setStatus] = useState<AttendanceStatus>(
        record?.status ?? AttendanceStatus.Invited
    );
    ...
    const handleSave = () => {
        ...
        onUpdate(
            status,
            showDeclineNote ? declineNote : undefined,
            showPayment ? paymentStatus : undefined
        );
    };
                ...
                {showDeclineNote && (
                    <div>
                        ...
                        {requireDeclineReason && !declineNote.trim() && (
                            <p className="text-xs text-error mt-1">Reason is required</p>
                        )}
                    </div>
                )}
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted">
                        ...
                        <span>
                            ...
                            {record.updatedByUserName && (
                                <span className="text-white/50"> by {record.updatedByUserName}</span>
                            )}
                        </span>
                    </div>
                )}
