...
const DRILLS: Drill[] = [
	{
		...
		impacts: [
			{ category: "Physical", skill: "Explosiveness", weight: 1.0 },
		],
	},
	{
		id: "d2",
		...
		description:
			"Continuous passing movement. Focus on footwork and platform.",
		impacts: [
...
export default function EvaluationPage() {
	...
	const stats = useMemo(() => {
		...
		return {
			Physical: totals.Physical.weight
				? (totals.Physical.sum / totals.Physical.weight).toFixed(1)
				: "-",
			Technical: totals.Technical.weight
				? (totals.Technical.sum / totals.Technical.weight).toFixed(1)
				: "-",
			Mental: totals.Mental.weight
				? (totals.Mental.sum / totals.Mental.weight).toFixed(1)
				: "-",
		};
	}, [drillScores]);
		...
		<div className="h-screen bg-background-dark text-white font-sans flex flex-col lg:flex-row overflow-hidden">
			...
			<div className="hidden xl:flex w-80 bg-[#1E1E1E] border-l border-white/5 flex-col shadow-2xl z-20 shrink-0">
				...
				<div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
					...
					<StatCard
						label="Physical"
						score={stats.Physical}
						icon={Dumbbell}
						color="text-blue-400"
						bg="bg-blue-500"
						desc="Agility, Jump, Power"
					/>
					...
					<div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-gray-800 to-black border border-white/10 text-center shadow-lg">
						...
						<div className="text-5xl font-black text-white tracking-tighter">
							...
							{(
								(Number(
									stats.Physical === "-" ? 0 : stats.Physical
								) +
									Number(
										stats.Technical === "-"
											? 0
									...
									Number(
										stats.Mental === "-" ? 0 : stats.Mental
									)) /
								(Object.values(drillScores).length > 0 ? 3 : 1)
							).toFixed(0)}
						</div>
						<div className="mt-2 inline-block px-3 py-1 rounded bg-accent text-white text-xs font-bold">
							PROSPECT
						</div>
					</div>
				</div>
	...
	return (
		<div>
			<div className="flex items-center gap-2 mb-2">
				...
				<span
					className={`ml-auto font-mono font-bold text-xl ${
						isRated ? "text-white" : "text-gray-600"
					}`}>
					{score}
				</span>
			</div>
