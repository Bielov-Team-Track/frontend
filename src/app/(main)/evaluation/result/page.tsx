...
export default function PlayerReportCard() {
	return (
		<div className="min-h-screen bg-background text-foreground font-sans p-4 md:p-8 flex justify-center pb-20">
			{/* --- THE CARD CONTAINER --- */}
			<div className="w-full max-w-4xl bg-background border border-border rounded-3xl overflow-hidden shadow-2xl relative">
				{/* Background Effects */}
				...
				{/* Header */}
				<div className="p-6 md:p-8 border-b border-border flex flex-col md:flex-row justify-between md:items-center gap-4">
					<div>
						<h1 className="text-2xl md:text-3xl font-bold text-foreground mb-1">Performance Report</h1>
					...
					<div className="flex gap-2 self-start md:self-auto">
						<button className="btn btn-sm btn-ghost hover:bg-hover text-muted hover:text-foreground">
							<Download size={18} />
						</button>
						<button className="btn btn-sm btn-ghost hover:bg-hover text-muted hover:text-foreground">
							<Share2 size={18} />
				...
				<div className="grid grid-cols-1 md:grid-cols-12 gap-0">
					...
					<div className="md:col-span-5 p-8 border-b md:border-b-0 md:border-r border-border flex flex-col items-center justify-center bg-surface">
						{/* Overall Rating Badge */}
						<div className="relative mb-8 text-center">
							<div className="w-32 h-32 rounded-full border-4 border-border bg-background flex items-center justify-center relative shadow-[0_0_30px_rgba(249,115,22,0.2)]">
								<div>
									<div className="text-5xl font-black text-foreground tracking-tighter">{REPORT.overallScore}</div>
									<div className="text-[10px] text-muted uppercase font-bold mt-1 tracking-widest">OVR</div>
								</div>
							</div>
							<div className="absolute -top-2 -right-2 w-10 h-10 bg-accent text-accent-foreground font-bold text-lg flex items-center justify-center rounded-lg border-4 border-surface shadow-lg">
								{REPORT.rank}
							</div>
						</div>
						...
						<div className="relative w-64 h-64">
							...
							<svg viewBox="0 0 100 100" className="w-full h-full text-gray-700 overflow-visible">
								...
								{/* Labels */}
								<text x="50" y="-2" textAnchor="middle" className="fill-muted-foreground" fontSize="6">
									SPIKE
								</text>
								<text x="98" y="25" textAnchor="start" className="fill-muted-foreground" fontSize="6">
									BLOCK
								</text>
								<text x="98" y="80" textAnchor="start" className="fill-muted-foreground" fontSize="6">
									SERVE
								</text>
								<text x="50" y="105" textAnchor="middle" className="fill-muted-foreground" fontSize="6">
									REC
								</text>
								<text x="2" y="80" textAnchor="end" className="fill-muted-foreground" fontSize="6">
									SPEED
								</text>
								<text x="2" y="25" textAnchor="end" className="fill-muted-foreground" fontSize="6">
									SET
								</text>
							</svg>
						</div>
					...
					<div className="md:col-span-7 p-8">
							...
							{Object.entries(REPORT.stats).map(([key, value]) => (
								<div key={key} className="group">
									<div className="flex justify-between text-sm mb-1">
										...
										<span className="text-foreground font-bold">{value}/10</span>
									</div>
						...
						<div className="bg-surface rounded-xl p-5 border border-border">
							<div className="flex items-center gap-3 mb-3">
								<div className="w-8 h-8 rounded-full bg-surface-elevated border border-border"></div>
								<div>
									<div className="text-sm font-bold text-foreground">{REPORT.coachName}</div>
									<div className="text-xs text-muted">Head Coach</div>
								</div>
							</div>
							<div className="relative">
								<span className="absolute -top-2 -left-1 text-4xl text-foreground/10 font-serif">"</span>
