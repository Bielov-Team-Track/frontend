...
const PaymentsSection = async ({ event, teams, userProfile }: PaymentsSectionProps) => {
	...
	const userPayment = userProfile?.id ? await loadUserPaymentForEvent(event.id!, userProfile.id) : null;

	const captainsTeams = teams.filter((t) => t.captainId === userProfile?.id);
	...
	const getPaymentStatus = (payment?: Payment | null, iconOnly: boolean = false) => {
			...
			<div className=" flex flex-col w-full gap-2">
				...
				{captainsTeams &&
					captainsTeams.map(async (t, index) => {
						...
						const remainingAmount = payments.reduce((sum, p) => sum + (p.paidAt ? 0 : p.amount), 0);

						return (
							<div key={t.id} className="flex flex-col gap-2 p-4 rounded-lg bg-background">
								<div className="flex items-center justify-between">
								...
								<div key={t.id} className="mt-2">
									<div className="text-sm text-muted font-medium">Team members</div>
