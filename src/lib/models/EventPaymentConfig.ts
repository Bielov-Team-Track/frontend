...
export interface EventPaymentConfig {
	...
	minUnitsForPaymentConfig?: number | undefined; // Safety minimum for payment config split
	// Timing rules
	dropoutDeadlineHours?: number | undefined; // Hours before event when registrations lock (for payment config split)
	// Payment reminder (for pay-to-join events)
	paymentReminderDaysBefore?: number | null; // Days before event to send payment reminder
}

export interface CreateEventPaymentConfig {
	name: string;
	description?: string;
	pricingModel: PricingModel;
	cost: number;
	minUnitsForPaymentConfig?: number;
