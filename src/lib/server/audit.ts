...
export const getEventsByFilter = async (filter?: EventFilterRequest): Promise<Event[] | null> => {
...
export const getOutstandingBalance = async (): Promise<BalanceAmount | null> => {
	const cookieStore = cookies();
	const token = (await cookieStore).get("token")?.value;
	if (!token) {
		return null;
	...
	try {
		...
		if (response.ok) {
			...
			console.error("Error fetching Outstanding Balance:", response.statusText);
			return null;
		}
	} catch (error) {
