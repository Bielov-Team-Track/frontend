...
class TeamManager {
	rotate = (team: Team) => {
		...
		if (team.rotation[3].name == "Libero") {
			const currentMiddle = team.rotation.find((p) =>
				p.name.includes("Middle"),
			);
			team.rotation[3] = team.positions.find(
				(p) => p.name.includes("Middle") && p.id != currentMiddle?.id,
			)!;
		}
	};
...
export default class GameEngine {
	...
	checkGameEnd = () => {
		...
		if (
			Math.abs(currentGameState.team1.points - currentGameState.team2.points) >
				1 &&
			(currentGameState.team1.points >= 25 ||
				currentGameState.team2.points >= 25)
		) {
			return true;
		}
