...
const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(({
  ...
  return (
    <div className={`form-control ${fullWidth ? 'w-full' : 'w-auto'}`}>
      ...
      <div className="relative">
        ...
        {(showCharCount || maxLength) && (
          <div className="absolute bottom-2 right-2 pointer-events-none">
            <span className={`${responsiveClasses.text.caption} text-base-content/50 bg-base-100 px-1 rounded`}>
              {characterCount}
              {maxLength && `/${maxLength}`}
            </span>
          </div>
      ...
      {helperText && !error && (
        <div className="mt-1">
          <span className={`${responsiveClasses.text.caption} text-base-content/70`}>
            {helperText}
          </span>
        </div>
      )}
