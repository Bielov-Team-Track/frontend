...
const meta: Meta<typeof ImageCropper> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'An image cropping component using react-easy-crop. Allows users to crop images to a 1:1 aspect ratio.',
      },
    },
  },
  tags: ['autodocs'],
      ...
      {selectedFile && (
        <div className="space-y-4">
          ...
          <div className="border rounded-lg p-4 bg-gray-50">
            <ImageCropper imageFile={selectedFile} onImageSave={handleImageSave} />
          </div>
        </div>
      )}
...
export const WithSampleImage: Story = {
  render: () => {
    ...
    const createSampleFile = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          const mockFile = new File([blob], 'sample-for-cropping.png', { type: 'image/png' })
          setFile(mockFile)
        }
      })
    }
    ...
    return (
      <div style={{ width: '500px' }}>
        <ImageCropper 
          imageFile={file} 
          onImageSave={(blob: Blob) => console.log('Saved cropped image:', blob)} 
        />
      </div>
    )
  },
