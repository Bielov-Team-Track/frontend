					...
					{hasSubItems ? (
						<>
							<Link
								...
								/>
								<span className="hidden xl:block flex-1 ml-4 font-medium text-sm">
									{item.name}
								</span>
							</Link>
			...
			{hasSubItems ? (
				<div
					...
					<button
						...
						className="text-muted/50 hover:text-white p-1 -mr-1">
						{itemExpanded ? (
							<ChevronDown size={12} />
						) : (
		...
		<aside className="w-20 xl:w-72 h-[calc(100vh-2rem)] sticky top-4 z-40 flex flex-col bg-background/50 backdrop-blur-xl border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
			...
			<div className="h-24 flex items-center justify-center xl:justify-start xl:px-8 border-b border-white/5">
				<div className="flex items-center gap-3">
					...
					<span className="hidden xl:block text-2xl font-bold text-white tracking-tight">
						Volleyer
					</span>
				</div>
			</div>
