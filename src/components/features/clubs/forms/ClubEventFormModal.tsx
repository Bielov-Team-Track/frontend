...
export default function ClubEventFormModal({
	isOpen,
	teams,
	groups,
	onClose,
	onSubmit,
}: ClubEventFormModalProps) {
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		onSubmit({
			...
			targetType,
			targetTeamIds:
				targetType === "teams" || targetType === "custom"
					? selectedTeamIds
					: undefined,
			targetGroupIds:
				targetType === "groups" || targetType === "custom"
					? selectedGroupIds
					: undefined,
		});
		...
		<Modal isOpen={isOpen} onClose={onClose} title="Create Event" size="lg">
			<form onSubmit={handleSubmit} className="space-y-5">
				...
				<Input
					...
					placeholder="e.g., Main Gym, Beach Court"
				/>
				...
				{targetType === "teams" && teams.length === 0 && (
					<div className="text-center py-4 text-muted text-sm">
						No teams available. Create teams first to target them.
					</div>
				)}
				{targetType === "groups" && groups.length === 0 && (
					<div className="text-center py-4 text-muted text-sm">
						No groups available. Create groups first to target them.
					</div>
				)}
