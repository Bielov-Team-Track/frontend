...
const ReviewSection = ({ title, icon: Icon, onEdit, children }: ReviewSectionProps) => (
	<div className="p-3 sm:p-4 rounded-xl bg-white/5 border border-white/5 relative group transition-all hover:bg-white/10">
		<div className="flex items-center justify-between mb-1.5 sm:mb-2">
			<div className="flex items-center gap-1.5 sm:gap-2">
				<Icon size={12} className="text-accent sm:hidden" />
				<Icon size={14} className="text-accent hidden sm:block" />
				<span className="text-[10px] sm:text-xs text-muted uppercase tracking-wider">{title}</span>
			</div>
			<button
				onClick={onEdit}
				className="text-muted hover:text-white transition-colors p-1 sm:p-1.5 rounded-md hover:bg-white/10 sm:opacity-0 sm:group-hover:opacity-100 focus:opacity-100"
				title="Edit"
				type="button">
				<Pencil size={12} />
			</button>
	...
	return (
		<div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-right-4 duration-300" data-testid="review-step">
			<div className="border-b-2 pb-3 sm:pb-4">
				<h2 className="text-lg sm:text-xl font-bold text-white mb-1">Review & Create</h2>
				<p className="text-muted text-xs sm:text-sm">Double check everything before creating your event.</p>
			</div>
			...
			<div className="relative w-full rounded-xl sm:rounded-2xl overflow-hidden bg-surface-elevated border border-white/10 shadow-xl">
				...
				<div className="h-16 sm:h-24 w-full bg-linear-to-br from-accent/30 via-accent/10 to-transparent relative">
					...
					<button
						onClick={() => goToStep(1)}
						className="absolute top-2 right-2 sm:top-4 sm:right-4 p-1.5 sm:p-2 bg-black/20 hover:bg-black/40 backdrop-blur-md rounded-lg text-white/80 hover:text-white transition-all"
				...
				<div className="px-3 sm:px-6 pb-4 sm:pb-6 relative -mt-6 sm:-mt-8">
					...
					<div className="mb-4 sm:mb-6">
						<h3 className="text-xl sm:text-2xl font-bold text-white mb-1">{values.name || "Untitled Event"}</h3>
					...
					<div className="pt-3 sm:pt-4 border-t border-white/5 relative group">
						...
						<div className="flex flex-wrap gap-2 sm:gap-3">
							...
							<div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-white/5 border border-white/10">
								...
								<span className="text-[10px] sm:text-xs text-white">{!values.usePaymentConfig ? "Free" : getPaymentConfigSummary()}</span>
							</div>
							...
							<div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-white/5 border border-white/10">
								{values.isPublic ? (
									...
									<>
										<EyeOff size={10} className="text-muted sm:hidden" />
										<EyeOff size={12} className="text-muted hidden sm:block" />
									</>
								)}
								<span className="text-[10px] sm:text-xs text-white">{values.isPublic ? "Public event" : "Private event"}</span>
							</div>
						</div>
					</div>
				</div>
