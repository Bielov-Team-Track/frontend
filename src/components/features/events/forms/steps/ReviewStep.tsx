		...
		<div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
			...
			<div className="relative w-full rounded-2xl overflow-hidden bg-background-light border border-white/10 shadow-xl">
				...
				<div className="px-6 pb-6 relative -mt-8">
					{/* Event Title */}
					<div className="mb-6">
						<h3 className="text-2xl font-bold text-white mb-1">{values.name || "Untitled Event"}</h3>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
						...
						{values.location?.address && (
							<ReviewSection title="Location" icon={MapPin} onEdit={() => goToStep(3)}>
								<p className="text-sm text-white font-medium">{values.location.name || values.location.address}</p>
							</ReviewSection>
						)}
					</div>
					...
					<div className="pt-4 border-t border-white/5 relative group">
						<div className="absolute right-0 top-4 opacity-0 group-hover:opacity-100 transition-opacity">
							<button onClick={() => goToStep(4)} className="text-muted hover:text-white p-1.5" title="Edit Settings">
								<Pencil size={12} />
							</button>
						</div>
