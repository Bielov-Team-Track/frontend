...
import { CreateOrUpdateCoachProfileDto, CreateQualificationDto, SkillLevel } from "@/lib/models/Profile";
...
const CoachInfoStep = ({ defaultValues, onNext, formId }: Props) => {
	...
	const initialQualifications: Qualification[] =
		defaultValues?.qualifications?.map((qual, index) => ({
			id: index.toString(),
			name: qual.name,
			year: qual.year,
		})) || [];
	...
	const [addQualError, setAddQualError] = useState<string>();
	...
	const {
		...
		defaultValues: {
			...
			highestLevelCoached:
				defaultValues?.highestLevelCoached !== undefined && defaultValues?.highestLevelCoached !== null
	...
	const onSubmit = (data: FormData) => {
		...
		onNext({
			...
			highestLevelCoached: data.highestLevelCoached ? skillLevelStringToEnum[data.highestLevelCoached] : undefined,
			...
			<div className="flex flex-col gap-6">
				...
				<div className="flex flex-col gap-3">
					<label className="block text-sm font-medium text-white">
						Qualifications <span className="text-muted font-normal text-xs ml-1">(optional)</span>
					</label>
					...
					<div className="p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col gap-3">
						...
						<Button type="button" variant="outline" size="sm" onClick={handleAddQualification} className="self-end px-6">
							Add
						</Button>
					</div>
