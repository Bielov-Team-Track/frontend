...
import { Button, Input, MultiSelectPills, RadioCards, Select, Steps, TextArea } from "@/components";
import { Modal } from "@/components/ui";
import { useAddDrillAttachment, useCreateDrill } from "@/hooks/useDrills";
import {
	CreateDrillRequest,
	DrillAttachmentTypeEnum,
	DrillCategory,
	DrillCategoryEnum,
	DrillEquipmentInput,
	DrillIntensity,
...
const VISIBILITIES: DrillVisibility[] = ["Public", "Private"];
...
// Skills with icons for MultiSelectPills
const SKILL_OPTIONS = [
...
export default function CreateDrillModal({ isOpen, onClose, onSuccess }: CreateDrillModalProps) {
	...
	const handleSubmit = async () => {
		...
		try {
			const createdDrill = await createDrill.mutateAsync(request);
					...
					attachments.map((attachment) =>
						addAttachment.mutateAsync({
							...
							data: {
								...
								fileType: DrillAttachmentTypeEnum[attachment.fileType],
	...
	return (
		<Modal isOpen={isOpen} onClose={onClose} size="lg" showCloseButton={true}>
			<div className="flex flex-col h-full">
				{/* Header */}
				<div className="mb-6">
					<h2 className="text-xl font-bold text-white mb-1">Create Drill</h2>
					<p className="text-muted text-sm">Add a new drill to your library</p>
				</div>
			...
			<div className="max-w-xl m-auto mb-8">
				...
				<div className="flex-1 overflow-y-auto pr-1 min-h-[400px]">
					...
					{currentStep === 4 && (
						<StepMedia variations={variations} setVariations={setVariations} attachments={attachments} setAttachments={setAttachments} />
					)}
				</div>
...
function StepBasics({ name, setName, description, setDescription, category, setCategory, intensity, setIntensity, errors = {} }: StepBasicsProps) {
	return (
		<div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
			...
			<RadioCards label="Category" options={CATEGORY_OPTIONS} value={category} onChange={setCategory} columns={3} size="sm" />

			<RadioCards label="Intensity" options={INTENSITY_OPTIONS} value={intensity} onChange={setIntensity} columns={3} size="sm" />
		</div>
	);
}
	...
	return (
		<div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
			...
			<MultiSelectPills
				...
				helperText={
					selectedSkills.length === 0 ? "Select at least one skill" : `${selectedSkills.length} skill${selectedSkills.length > 1 ? "s" : ""} selected`
				}
				error={errors.skills}
			/>
...
interface StepMediaProps {
	...
	setVariations: (variations: StepMediaProps["variations"]) => void;
	...
	return (
		<div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
			...
			<div>
				...
				<DrillSelector selectedDrills={variations} onChange={setVariations} />
			</div>
		</div>
	);
}
...
function EquipmentList({ items, onChange }: EquipmentListProps) {
	...
	const toggleOptional = (index: number) => {
		onChange(items.map((item, i) => (i === index ? { ...item, isOptional: !item.isOptional } : item)));
	};
