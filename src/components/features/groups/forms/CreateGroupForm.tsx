      ...
      <div className='flex gap-2 items-center'>
        <Controller
          ...
          render={({ field: { value, onChange, ...rest } }) => (
            <input className='checkbox' type='checkbox' checked={!!value} onChange={(e) => onChange(e.target.checked)} {...rest} />
          )}
        />
        <label className='font-bold'>Approve Members</label>
        {errors.approveMembers && <p className="text-error">{(errors as any).approveMembers?.message}</p>}
      </div>
